jQuery(function($){
	...
	if(course_map_vars.mapping_settings != null && course_map_vars.mapping_settings != ""){
		mapping_settings = $.parseJSON(course_map_vars.mapping_settings);
	}
	else{
	...
	function save_configuration(fullConfig){
		$('#ax_course_mapping_settings').val(JSON.stringify(fullConfig));
	}
	$.axWidget("axcelerate.mapping_config",{
		...
		_buildMainUI:function(){
			...
			mapEditHolder.append(mapW._inputFieldHolder('ax_map_1', "Select Course").append(mapList1Select));
	...
	$('#ax_mapping_widget_holder').mapping_config({
		...
		mapping_list_1:course_map_vars.mapping_list_1,
		mapping_list_2:course_map_vars.mapping_list_2,
	});
